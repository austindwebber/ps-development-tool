...
function generateScript {
    ...
    if ($outsideofWD -eq "true" -and $complete -match 'false') {
        ...
        "`n#Date Created " + $currentDate + "`n" +'Write-Host -ForegroundColor Green "Started your installation..."' + "`n"  + "
        `n#Silently installs " + $applicationName + "`n" + '$parent = Split-Path -Path "$PSScriptRoot"' + "`nStart-Process" + ' "$parent\' + $installationFile + '"' + " -Wait -ArgumentList " + '"' + $switches + '"' +
        "`n`n" + 'Write-Host -ForegroundColor Green "Done!"' +"`n" + 'Start-Sleep -s 5') -Force | Out-Null
    ...
    if ($outsideofWD -ne "true") {
        ...
        if ($installationFileLocation -match '.exe' -and $complete -match 'false') {
            ...
            "`n#Date Created " + $currentDate + "`n" +'Write-Host -ForegroundColor Green "Started your installation..."' + "`n"  + "
            `n#Silently installs " + $applicationName + "`n" + '$parent = Split-Path -Path "$PSScriptRoot"' + "`nStart-Process" + ' "$parent\' + $installationFile + '"' + " -Wait -ArgumentList " + '"' + $switches + '"' +
             "`n" + 'Write-Host -ForegroundColor Green "Done!"' +"`n" + 'Start-Sleep -s 5') -Force | Out-Null

            #Copy over shortcut into Start Menu
            if ($startMenu -eq "y") {     
        
                #Add to install script
                Add-Content $(".\install_" + $applicationName + ".ps1") -Value $("`n#Copies a shortcut into the start menu" + "`n" + 'Copy-Item "'+ '$PSScriptRoot' + '\' + $startMenuShortcut + '" "' + '$env:ProgramData\Microsoft\Windows\Start Menu\Programs' + '" -Force') -Force | Out-Null
